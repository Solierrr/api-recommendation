...
DESTRUCTIVE_OPT_IN = os.getenv("NEO4J_INTEGRATION_ALLOW_DESTRUCTIVE", "").lower() == "true"
...
async def test_real_neo4j_lock_activation_validation_and_cleanup(monkeypatch) -> None:
    ...
    try:
        async with driver.session() as session:
            ...
            versions = {record["version"]: record["node_count"] async for record in result}
            ...
            blocked = [result for result in contenders if isinstance(result, SyncInProgressError)]
